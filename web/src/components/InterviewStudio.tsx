...
export function InterviewStudio() {
    ...
    if (step === 'report') return <main className="mx-auto max-w-screen-md px-4 pb-10 pt-20">
        ...
        <section className="mt-4 space-y-3"><h3 className="font-semibold text-ink dark:text-ink-dark">逐题反馈与参考答案</h3>{turns.map((item, index) => <article key={`${item.question.id}-${index}`} className="rounded-xl border border-hairline bg-canvas p-4 dark:border-hairline-dark dark:bg-canvas-dark"><p className="mb-1 text-xs text-primary">第 {index + 1} 题 · {item.question.focus} · {item.score}/4</p><h4 className="text-sm font-medium text-ink dark:text-ink-dark">{item.question.title}</h4><p className="mt-2 text-sm text-body dark:text-body-dark">{item.assessment}</p><div className="mt-3 rounded-lg bg-primary/5 p-3"><p className="text-xs font-medium text-primary">参考答案 / 作答要点</p><p className="mt-1 text-sm text-body dark:text-body-dark">{getInterviewAnswerGuide(item.question)}</p></div><p className="mt-2 text-xs text-muted dark:text-muted-dark">面试官决策：{item.decision}</p></article>)}</section><button onClick={() => setStep('setup')} className="mt-6 w-full rounded-xl bg-primary py-3 text-sm font-medium text-white">再进行一次模拟面试</button>
    </main>
