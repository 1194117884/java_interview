...
import { createLearningPlan, loadMemories, saveMemories, type SkillMemory } from '../lib/interviewEngine'

export function SkillMemoryProfile() {
    const [memories, setMemories] = useState<SkillMemory[]>(loadMemories)
    const learningPlan = createLearningPlan(memories)
    ...
    return <main className="mx-auto max-w-screen-md px-4 pb-10 pt-20">
        ...
        {learningPlan.length > 0 && <section className="mt-4 rounded-2xl border border-hairline bg-card p-5 dark:border-hairline-dark dark:bg-card-dark"><h3 className="font-semibold text-ink dark:text-ink-dark">下一轮学习计划</h3><div className="mt-3 space-y-3">{learningPlan.map(item => <article key={item.skill} className="rounded-lg bg-soft p-3 dark:bg-soft-dark"><div className="flex items-center justify-between gap-2"><p className="text-sm font-medium text-ink dark:text-ink-dark">{item.skill}</p><span className={`rounded-full px-2 py-0.5 text-xs ${item.priority === '高' ? 'bg-red-500/10 text-red-600 dark:text-red-400' : 'bg-primary/10 text-primary'}`}>{item.priority}优先级</span></div><p className="mt-2 text-xs text-body dark:text-body-dark">{item.action}</p></article>)}</div></section>}
